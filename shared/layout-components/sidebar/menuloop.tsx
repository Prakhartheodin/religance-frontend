...
function Menuloop({ local_varaiable ,MenuItems, toggleSidemenu, level = 1 , HoverToggleInnerMenuFn}: any) {
    ...
    <Fragment>
        ...
        onClick={(event) => { event.preventDefault(); toggleSidemenu(event, MenuItems); }} onMouseEnter={ (event) =>HoverToggleInnerMenuFn(event, MenuItems)}>
         ...
         {local_varaiable?.dataVerticalStyle != "doublemenu" ? MenuItems.icon :""}
          ...
          <span className="side-menu__label">
            {MenuItems.title}
            {MenuItems.badgetxt ? (<span className={MenuItems.class}> {MenuItems.badgetxt} </span>) : null}
          </span>
      ...
      >
        ...
        {MenuItems.children.map((firstlevel: any, index:any) =>
          <li className={`${firstlevel.type === 'category' || firstlevel.menutitle ? 'slide__category' : ''} ${firstlevel?.type == 'empty' ? 'slide' : ''} ${firstlevel?.type == 'link' ? 'slide' : ''} ${firstlevel?.type == 'sub' ? 'slide has-sub' : ''} ${firstlevel?.active ? 'open' : ''} ${firstlevel?.selected ? 'active' : ''}`} key={index}>
