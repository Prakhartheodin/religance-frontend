...
const Sidebar = ({ local_varaiable, ThemeChanger }: any) => {
	...
	function isSettingsSubRoute(currentPath: string) {
		return (
	...
	/** Open collapsible submenus (e.g. Settings) from route unless user collapsed them. */
	function syncCollapsibleSubmenus(
		items: any = menuitems,
		currentPath?: string
	) {
		items?.forEach((item: any) => {
			if (item?.menutitle) return;
			if (item?.type === "sub" && item?.collapsible && item.children?.length) {
				if (currentPath !== undefined) {
					if (settingsSubmenuCollapsedByUser.current) {
						item.active = false;
					} else if (isSettingsSubRoute(currentPath)) {
						item.active =
							currentPath === "/settings" ||
							item.children.some(
			...
			if (item?.children?.length) {
				syncCollapsibleSubmenus(item.children, currentPath);
			}
		});
	}
	...
	function closeMenu() {
		...
		const currentPath = pathname.endsWith("/")
			? pathname.slice(0, -1)
			: pathname;
		syncCollapsibleSubmenus(menuitems, currentPath);
	...
	useEffect(() => {
		...
		if (currentPath !== previousUrl) {
			settingsSubmenuCollapsedByUser.current = false;
		...
		if ((theme.dataNavStyle != "icon-hover" && theme.dataNavStyle != "menu-hover") || (window.innerWidth < 992) || (theme.dataNavLayout != "horizontal") && (theme.dataToggled != "icon-hover-closed" && theme.dataToggled != "menu-hover-closed")) {
			...
			for (const item of MenuItems) {
				if (item === targetObject) {
					...
					if (item.collapsible) {
						settingsSubmenuCollapsedByUser.current = !item.active;
					}

					if (item.active) {
						closeOtherMenus(MenuItems, item, 0);
	...
	function closeOtherMenus(MenuItems: any, targetObject: any, depth = 0) {
		for (const item of MenuItems) {
			if (item !== targetObject) {
				item.active = false;
				if (item.children && item.children.length > 0) {
					closeOtherMenus(item.children, targetObject, depth + 1);
				}
			}
		}
	}
		...
		for (const mutation of mutationsList) {
			if (mutation.type === 'attributes' && mutation.attributeName === 'data-nav-layout') {
				...
				if (newValue == 'vertical') {
					let currentPath = pathname.endsWith('/') ? pathname.slice(0, -1) : pathname;
					currentPath = !currentPath ? CRM_HOME_PATH : currentPath;
					setMenuUsingUrl(currentPath);
				} else {
					closeMenu();
				}
								...
								{menuitems.map((levelone: any, index:any) => (
									<Fragment key={index}>
										<li className={`${levelone.menutitle ? 'slide__category' : ''} ${levelone.type === 'link' ? 'slide' : ''}
                                               ${levelone.type === 'sub' ? 'slide has-sub' : ''} ${levelone?.collapsible ? 'slide--collapsible' : ''} ${levelone?.active ? 'open' : ''} ${levelone?.selected ? 'active' : ''}`}>
											{levelone.menutitle ?
											...
											{levelone.type === "empty" ?
												...
												: ""}
											{levelone.type === "sub" ?
												<Menuloop MenuItems={levelone} level={1} toggleSidemenu={toggleSidemenu} HoverToggleInnerMenuFn={HoverToggleInnerMenuFn} />
												: ''}
										</li>
									</Fragment>
								))}
